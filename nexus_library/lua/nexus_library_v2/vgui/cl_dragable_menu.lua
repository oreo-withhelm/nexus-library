...
function PANEL:Init()
    ...
    self.ScaleBar.TextArea:Hide()
    self.ScaleBar.Slider:Dock(FILL)
    self.ScaleBar.Slider.Paint = function(s, w, h)
        Nexus.RNDX.Draw(Nexus:GetMargin(), 0, 0, w, h, Nexus:GetColor("highlight"))
    end
    self.ScaleBar.Slider.Knob.Paint = function(s, w, h)
        Nexus.RNDX.Draw(Nexus:GetMargin(), 0, 0, w, h, Nexus:GetColor("primary"))
    end
    ...
    self.ResetButton = self:Add("Nexus:V2:Button")
    self.ResetButton:SetPos(Nexus:GetScale(155), 0)
    self.ResetButton:SetSize(Nexus:GetScale(20), Nexus:GetScale(20))
    self.ResetButton:SetText("")
    self.ResetButton:SetColor(Nexus:GetColor("green"))
