...
function Nexus:GetScale(number)
    ...
    value = value * ((Nexus:GetSetting("nexus_UIScale", 100) or 100) / 100)
    cachedScales[number] = value

    return math.Round(value)
end

hook.Add("Nexus:Accessibility", "Nexus:ResetCache", function()
    cachedScales = {}
end)
...
function Nexus:DermaMenu(tbl, onClicked, noSort, panel, alignLeft)
    ...
    for k, v in ipairs(tbl) do
        ...
        button:SetTextColor(Nexus:GetTextColor(Nexus:GetColor("overlay")))
...
function Nexus:QueryPopup(str, onYes, onNo, yesText, noText, stopBlur)
    if isstring(str) and isstring(onYes) and isfunction(onNo) then
        local title, text, onYesFunc = str, onYes, onNo
        Nexus:QueryPopup(text, onYesFunc, function() end, Nexus:GetPhrase("Yes", "nexus_lib"), "No")
        return
    end
    ...
    frame:SetSize(Nexus:GetScale(300), Nexus:GetScale(200))
    frame:Center()
    frame:MakePopup()
    ...
    frame.PaintOver = function(s, w, h)
        Nexus.RNDX.DrawOutlined(Nexus:GetMargin("large"), 0, 0, w, h, Nexus:GetColor("secondary-text"), 1)
    end
    Nexus.Popup = frame

    local label = frame:Add("DLabel")
    label:Dock(TOP)
    label:DockMargin(Nexus:GetMargin("large"), Nexus:GetMargin("large"), Nexus:GetMargin("large"), 0)
    label:SetText(str)
    ...
    if tw > Nexus:GetScale(300)-Nexus:GetMargin("large")*2 then
        label:SetWrap(true)
    end

    label.PerformLayout = function(s)
        frame:SetTall(Nexus:GetScale(35) + label:GetTall() + Nexus:GetMargin("large")*3)
    end

    local container = frame:Add("DPanel")
    container:Dock(BOTTOM)
    container:SetTall(Nexus:GetScale(35))
    container:DockMargin(Nexus:GetMargin("large"), 0, Nexus:GetMargin("large"), Nexus:GetMargin("large"))
    container.Paint = nil

    frame.yes = container:Add("Nexus:V2:Button")
    frame.yes:Dock(LEFT)
    frame.yes:DockMargin(0, 0, 0, 0)
    ...
    frame.no = container:Add("Nexus:V2:Button")
    frame.no:Dock(FILL)
    frame.no:DockMargin(Nexus:GetMargin(), 0, 0, 0)
    frame.no:SetText(noText)
    frame.no:SetSecondary()
...
local cachedAlphas = {}
function Nexus:SetAlpha(col, a)
    local key = string.format("%s_%d", tostring(col), a)
    if cachedAlphas[key] then
        return cachedAlphas[key]
    end

    cachedAlphas[key] = Color(col.r, col.g, col.b, a)
    return cachedAlphas[key]
end
