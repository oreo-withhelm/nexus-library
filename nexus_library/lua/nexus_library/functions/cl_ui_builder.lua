...
function BASE:CreateFrame(tbl)
    ...
    tbl.size.w = tbl.size.w or Nexus:GetScale(200)
    tbl.size.h = tbl.size.h or Nexus:GetScale(300)

    tbl.title = tbl.title or ""
    tbl.stopGradient = tbl.stopGradient
    ...
    if tbl.darken then
        ...
        background.Paint = function(s, w, h)
            Nexus.RNDX.Draw(0, 0, 0, w, h, bg, Nexus.RNDX.BLUR)
            Nexus.RNDX.Draw(0, 0, 0, w, h, bg)
        end
    end

    local frame = IsValid(background) and background:Add("Nexus:V2:Frame") or vgui.Create("Nexus:V2:Frame")
    frame:SetSize(tbl.size.w, tbl.size.h)
    frame:Center()
    frame:MakePopup()
    frame:SetTitle(tbl.title, tbl.icon)
    frame.stopGradient = tbl.stopGradient
...
function BASE:AddSpacer()
    ...
    spacer:SetBackgroundColor(Nexus:GetColor("primary-text"))
...
function BASE:AddBlock(tbl)
    ...
    tbl.color = tbl.color or Nexus:OffsetColor(Nexus:GetColor("background"), 20, true)
    ...
    panel.Paint = function(s, w, h)
        draw.RoundedBox(Nexus:Scale(10), 0, 0, w, h, tbl.color)
    end

    local scrollBar = panel:Add("Nexus:V2:HorizontalScrollPanel")
    scrollBar:Dock(FILL)
...
function BASE:AddButton(tbl)
    ...
    tbl.margin = tbl.margin or self.margin
    tbl.color = tbl.color or Nexus:GetColor("primary")
