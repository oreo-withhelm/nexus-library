...
function Nexus:LegacyPopup(title, text, callback)
    ...
    local frame = vgui.Create("Nexus:V2:Frame")
    frame:SetSize(Nexus:Scale(400), 0)
    frame:Center()
    frame:SetTitle(title)
    frame:MakePopup()

    frame:SetTall(frame:GetTall() + frame.Header:GetTall())
    ...
    frame:SetTall(frame:GetTall() + label:GetTall())
    ...
    frame:SetTall(frame:GetTall() + Nexus:GetMargin()*3 + Nexus:GetScale(35))
end
