...
local function ImgurDownload(id)
    local function onError()
        ...
        http.Fetch(proxyURL..id, function(b, len, head, code)
            if code != 200 or len == 34641 or len == 503 then print("[ Nexus ] Proxy: FAILED IMAGE SAVE: "..id) return end // not valid in our region
    ...
    http.Fetch( "https://i.imgur.com/" .. id .. ".png", function(b, len, head, code)
        if code != 200 or len == 34641 or len == 503 then onError() return end // not valid image
...
function Nexus:GetImgur( id )
    ...
    if self:LoadData( nil, "nexus_v2", string.lower(id), false, "png" ) then
        self.Materials[ id ] = Material( "../data/nexus_v2/" .. string.lower(id) .. ".png", "noclamp smooth mips" )
        return self.Materials[ id ]
    end

    if file.Exists("materials/nexus_v2/"..string.lower(id)..".png", "GAME") then
        self.Materials[ id ] = Material( "nexus_v2/"..string.lower(id)..".png", "noclamp smooth mips" )
        return self.Materials[ id ]
    end
