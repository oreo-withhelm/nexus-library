...
function PANEL:Init()
	...
	self:SetHeaderHeight(Nexus:GetScale(20))
...
function PANEL:FixColumnsLayout()
	...
	for k, Column in pairs(self.Columns) do
		...
		Column.PaintOver = function(s, w, h)
			Nexus.RNDX.Draw(Nexus:GetMargin(), 0, 0, w, h, Nexus:GetColor("primary"), Nexus.RNDX.NO_BL + Nexus.RNDX.NO_BR + Nexus.RNDX.NO_TL + Nexus.RNDX.NO_TR)
			draw.SimpleText(Column.Header:GetText(), Nexus:GetFont({size = 15}), w/2, h/2, Nexus:GetTextColor(Nexus:GetColor("primary")), 1, 1)

			if s.Header:IsHovered() then
				Nexus.RNDX.Draw(0, 0, 0, w, h, Nexus:GetColor("overlay"))
			end
		end
	end
end
...
function PANEL:AddLine(...)
	...
	Line.SetColumnText = function(s, i, strText)
        ...
        if (!IsValid(s.Columns[i])) then
            ...
            s.Columns[i]:SetTextColor(Nexus:GetColor("primary-text"))
			s.Columns[i]:SetFont(Nexus:GetFont({size = 14}))
        end
...
function PANEL:Paint(w, h)
    draw.RoundedBox(Nexus:GetMargin(), 0, 0, w, h, self.col or Nexus:GetColor("secondary"))
end
