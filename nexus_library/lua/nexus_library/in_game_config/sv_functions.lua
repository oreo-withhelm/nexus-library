    ...
    elseif elementType == "table" then
        ...
        if isAdd then
            ...
            for _, v in ipairs(isfunction(info.values) and info.values() or info.values) do
                ...
                elseif v.type == "ComboBox" then
                    ...
                    for _, item in ipairs(isfunction(v.values) and v.values() or v.values) do
                        if (istable(item) and item.value or item) == value then
                            hasFound = true
                            break
                        end
                    end

                    if !hasFound and not table.HasValue(isfunction(v.values) and v.values() or v.values, value) then
                        Nexus:Notify(ply, 1, 3, "Fill out all fields.")
                        return
                    end
            ...
            if not info.dontNetwork then
                ...
                for k, v in ipairs(isfunction(info.values) and info.values() or info.values) do
...
util.AddNetworkString("Nexus:IGC:V2:EditValue")
util.AddNetworkString("Nexus:IGC:V2:ValueEditted")
net.Receive("Nexus:IGC:V2:EditValue", function(len, ply)
    local column = net.ReadString()
    local mID = net.ReadUInt(32)
    local valueID = net.ReadString()

    local value;

    net.Start("Nexus:IGC:V2:ValueEditted")
    net.WriteString(valueID)
    net.WriteString(column)
    net.WriteUInt(mID, 32)

    local info = GetElementInfo(valueID)
    for _, v in ipairs(info.values) do
        ...
        if v.type == "TextEntry" and v.isNumeric then
            ...
            net.WriteUInt(0, 2)
            net.WriteString(value)
        elseif v.type == "TextEntry" or v.type == "ComboBox" then
            value = net.ReadString()
            if not value then continue end
            net.WriteUInt(1, 2)
            net.WriteString(value)
    ...
    if value == nil then
        net.Abort()
        Nexus:Notify(ply, 0, 3, "ERROR")
        return
    end

    local data = Nexus:GetValue(valueID)
    for _, v in ipairs(data) do
        if v["m_id"] ~= mID then continue end
        v[column] = value
    end

    Nexus:SetValue(valueID, data)

    if not Nexus:GetValue("nexus-config-admins")[ply:GetUserGroup()] and not Nexus.Admins[ply:GetUserGroup()] then
        net.Abort()
        Nexus:Notify(ply, 1, 3, "Incorrect Usergroup")
        return
    end

    if info.dontNetwork then
        net.Abort()
    else
        net.Broadcast()
    end
