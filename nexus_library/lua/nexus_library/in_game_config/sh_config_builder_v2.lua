...
function BUILDER:AddButtons(tbl)
    ...
    table.Add(self.Elements, {{id = "button-row", data = tbl, label = tbl.label}})
...
function BUILDER:AddTextEntry(tbl)
    ...
    table.Add(self.Elements, {{id = "text-entry", data = tbl, label = tbl.label}})
...
function BUILDER:AddMultiTextEntry(tbl)
    ...
    table.Add(self.Elements, {{id = "multi-text-entry", data = tbl, label = tbl.label}})
...
function BUILDER:AddKeyTable(tbl)
    ...
    table.Add(self.Elements, {{id = "key-table", data = tbl, label = tbl.label}})
...
function BUILDER:AddTable(tbl)
    ...
    if tbl.isPercentage then
        table.Add(tbl.values, {{id = "Chance", type = "TextEntry", placeholder = "weight", isNumeric = true, label = "%"}})
    end

    table.Add(self.Elements, {{id = "table", data = tbl, label = tbl.label}})
